export namespace main {
	...
	export class RegionSelection {
	    ...
	    clickPoint: ClickPointInput;
	
	    static createFrom(source: any = {}) {
	        return new RegionSelection(source);
	    }
	
	    constructor(source: any = {}) {
	        if ('string' === typeof source) source = JSON.parse(source);
	        this.region = this.convertValues(source["region"], CaptureRegionInput);
	        this.clickPoint = this.convertValues(source["clickPoint"], ClickPointInput);
	    }
